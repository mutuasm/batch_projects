...
def create_share_link(project, scope="board", task=None, expires_in_days=None, label=None, access_level="view"):
    ...
    if scope == "task":
        ...
        if bpq.get_value(TASK(), task, "project") != project:
...
def get_shared(token):
    ...
    if link.scope == "task":
        if not link.task or not bpq.exists(TASK(), link.task):
...
def add_guest_comment(token, comment_text, guest_name=None):
    ...
    task_data = bpq.get_value(TASK(), link.task, ["is_deleted"], as_dict=True)
