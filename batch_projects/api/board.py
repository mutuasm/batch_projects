...
def query_tasks(project, filters=None, group_by=None, sort_by="creation",
    ...
    issues = bpq.get_all(TASK(), **kwargs)
...
def update_project_general(project, project_name=None, key=None, description=None,
    ...
    if key is not None:
        ...
        if key != doc.key:
            if bpq.exists(PROJECT(), {"key": key, "name": ["!=", project]}):
    ...
    if changes:
        ...
        bpq.set_value(PROJECT(), project, changes)
        frappe.db.commit()
...
def purge_project(project, confirm_key=None):
    ...
    key = bpq.get_value(PROJECT(), project, "project_name") or project
...
def _project_financial_blockers(project) -> list:
    ...
    task_names = bpq.get_all(TASK(), filters={"project": project}, pluck="name")
    if task_names:
...
def update_project_workflow(project, workflow_states):
    ...
    existing_raw = _deep_parse_json(
        bpq.get_value(PROJECT(), project, "workflow_states") or "[]"
    )
    ...
    bpq.set_value(PROJECT(), project, {
        "workflow_states": json.dumps(states),
        "schema_version": (bpq.get_value(PROJECT(), project, "schema_version") or 0) + 1,
...
def update_project_issue_types(project, issue_types):
    ...
    existing_raw = _deep_parse_json(
        bpq.get_value(PROJECT(), project, "issue_types") or "[]"
    )
    existing = existing_raw if isinstance(existing_raw, list) else []
...
def update_project_labels(project, labels):
    ...
    existing_raw = _deep_parse_json(
        bpq.get_value(PROJECT(), project, "labels") or "[]"
    )
...
def get_gantt(project):
    ...
    if not bpq.exists(PROJECT(), project):
        alt = bpq.get_value(PROJECT(), {"key": project}, "name")
        if alt:
            project = alt
    _check_permission(project, "BP Viewer")
...
def move_task(issue, status=None, prev=None, next=None, force=False):
    ...
    def _rank(name):
        return bpq.get_value(TASK(), name, "board_rank") if name else None
...
def _hard_delete_task(doc_or_name):
    ...
    for subtask in bpq.get_all(TASK(), filters={"parent_task": issue}, pluck="name"):
...
def delete_task(issue):
    ...
    for subtask in bpq.get_all(TASK(), filters={"parent_task": issue, "is_deleted": 0}, pluck="name"):
        delete_task(subtask)
...
def restore_task(issue):
    ...
    for subtask in bpq.get_all(TASK(), filters={"parent_task": issue, "is_deleted": 1}, pluck="name"):
...
def move_task_to_project(issue, target_project):
    ...
    bpq.set_value(TASK(), doc.name, "task_key", new_key)
...
def get_export_data(project, view=None):
    ...
    tasks = bpq.get_all(TASK(), filters={"project": project},
        fields=["name", "task_key", "title", "status", "priority", "task_type",
...
def get_mirror_values(doctype, names, project=None):
    ...
    if project and doctype in _MIRROR_PROJECT_SCOPED_DOCTYPES:
        _check_permission(project, "BP Viewer")
        erp_project = bpq.get_value(PROJECT(), project, "erpnext_project")
        if erp_project:
    ...
    if int(muted):
        if not existing:
            frappe.get_doc({
                ...
                "user": user,
                "task": task or None,
                "project": project or (bpq.get_value(TASK(), task, "project") if task else None),
...
def get_epics(project):
    ...
    for epic in epics:
        total = bpq.count(TASK(), _task_filters({"epic": epic["name"]}))
        done  = bpq.count(TASK(), _task_filters({"epic": epic["name"], "status": ["in", completed]}))
...
def delete_epic(epic):
    ...
    bpq.set_value(TASK(), {"epic": epic}, "epic", None, update_modified=False)
    doc.delete(ignore_permissions=True)
    from batch_projects.cache import invalidate_project
    invalidate_project(project)
    return {"ok": True}
...
def get_sprints(project):
    ...
    raw_states = bpq.get_value(PROJECT(), project, "workflow_states") or "[]"
...
def _resolve_scope(scope):
	...
	if isinstance(scope, list):
		...
		resolved = []
		for s in scope:
			p = s if bpq.exists(PROJECT(), s) else bpq.get_value(PROJECT(), {"key": s}, "name")
			if p:
	...
	# single project
	proj_name = scope if bpq.exists(PROJECT(), scope) else bpq.get_value(PROJECT(), {"key": scope}, "name")
	if proj_name:
...
def add_reference(issue, ref_doctype, ref_name, two_way=0):
    ...
    if int(two_way or 0):
        ...
        try:
            key = bpq.get_value(PROJECT(), doc.project, "key")
...
def archive_team(team):
	...
	for p in bpq.get_all(PROJECT(), filters={"team": team}, pluck="name"):
		bpq.set_value(PROJECT(), p, "team", None)
	frappe.db.commit()
	return {"ok": True}
...
def assign_project_to_team(project, team):
	...
	bpq.set_value(PROJECT(), project, "team", team or None)
	frappe.db.commit()
...
def backfill_reciprocal_links():
    ...
    for r in rows:
        ...
        if not inv or not bpq.exists(TASK(), tgt):
            continue
        if (src, inv) in existing.get(tgt, set()):
            continue  # reciprocal already present
        meta = bpq.get_value(TASK(), src, ["task_key", "title", "status"], as_dict=True)
        if not meta:
            continue
...
def complete_sprint(sprint, move_incomplete_to=None):
    ...
    emit(SPRINT_COMPLETED, {
        ...
        "sprint": doc.name,
        "sprint_name": doc.sprint_name,
        "completed_count": bpq.count(TASK(), {"sprint": sprint, "status": ["in", done_statuses]}) if done_statuses else 0,
...
def get_milestones(project=None):
	...
	proj_info = {}
	if proj_names:
		for p in bpq.get_all(PROJECT(), filters={"name": ["in", proj_names]},
...
def _get_completed_statuses_by_project(project_name: str) -> list:
    raw = bpq.get_value(PROJECT(), project_name, "workflow_states")
...
def delete_attachment(file_name):
    ...
    _check_permission(
        bpq.get_value(TASK(), doc.attached_to_name, "project"),
        "BP Member"
    )
    doc.delete(ignore_permissions=True)
    return {"ok": True}
...
def delete_sprint(sprint):
    ...
    project = doc.project
    task_names = bpq.get_all(TASK(), filters={"sprint": sprint}, fields=["name"])
    for t in task_names:
...
def get_allowed_doctypes(project=None):
    ...
    erp_linked = bool(project and bpq.get_value(PROJECT(), project, "erpnext_project"))
...
def get_reports(project, period="last_30_days", from_date=None, to_date=None):
	...
	if proj_names is None:
		...
		proj_names = bpq.get_all(PROJECT(), pluck="name") if acc is None else list(acc)
	else:
		# Normalise keys → names and permission-check each project in scope.
		resolved = []
		for s in proj_names:
			pn = s if bpq.exists(PROJECT(), s) else bpq.get_value(PROJECT(), {"key": s}, "name")
			if pn:
...
def set_parent_project(project, parent_project=None):
    ...
    if parent_project:
        ...
        check = bpq.get_value(PROJECT(), parent_project, "parent_project")
        visited = {project, parent_project}
        while check:
            if check in visited:
                frappe.throw("Circular parent relationship detected.")
            visited.add(check)
            check = bpq.get_value(PROJECT(), check, "parent_project")
...
def get_triage_queue(project=None):
    ...
    proj_map = {}
    if pnames:
        for p in bpq.get_all(PROJECT(), filters={"name": ["in", pnames]},
...
def _file_project_and_min_role(doc, min_role):
    ...
    if doc.attached_to_doctype == TASK():
        project = bpq.get_value(TASK(), doc.attached_to_name, "project")
...
def get_teams():
    ...
    for team in teams:
        ...
        projects = [] if proj_filters is NO_ACCESSIBLE_PROJECTS else bpq.get_all(
            PROJECT(),
            filters=proj_filters,
...
def get_team(team):
    ...
    data["projects"] = [] if _proj_filters is NO_ACCESSIBLE_PROJECTS else bpq.get_all(
        PROJECT(),
    ...
    for p in data["projects"]:
        p["open_count"] = bpq.count(TASK(), _task_filters({
            "project": p["name"],
...
def get_my_tasks(
    ...
    for task in tasks:
        ...
        if proj not in project_name_map:
            project_name_map[proj] = bpq.get_value(PROJECT(), proj, "project_name") or proj
    ...
    for task in tasks:
        ...
        if proj not in wf_color_map:
            states = _deep_parse_json(
                bpq.get_value(PROJECT(), proj, "workflow_states") or "[]"
            )
...
def get_workspace_summary():
    ...
    try:
        ...
        for m in ms_rows:
            ...
            open_t = bpq.count(TASK(), _task_filters({
                "project": m["project"],
...
def get_dashboard():
    ...
    for p in projects:
        ...
        p["open_count"]  = bpq.count(TASK(), _task_filters({"project": p["name"], "status": ["not in", completed or ["Done"]]}))
        p["total_count"] = bpq.count(TASK(), _task_filters({"project": p["name"]}))
        # Real members
        members = frappe.get_all(
            "BP Project Member",
...
def search_tasks_global(query, exclude=None, limit=12, offset=0):
    ...
    name_map = {}
    if proj_names:
        for p in bpq.get_all(PROJECT(), filters={"name": ["in", list(proj_names)]},
                                fields=["name", "project_name", "key"]):
            name_map[p["name"]] = p
    for r in rows:
...
def start_sprint(sprint):
    ...
    emit(SPRINT_STARTED, {
        ...
        "sprint": doc.name,
        "sprint_name": doc.sprint_name,
        "total_issues": bpq.count(TASK(), _task_filters({"sprint": doc.name})),
    })
    return doc.as_dict()
...
def watch_task(task):
    ...
    from batch_projects.task_validation import require_live_task
    _check_permission(bpq.get_value(TASK(), task, "project"), "BP Viewer")
...
def remove_task_link(issue, linked_task, link_type):
    ...
    inverse = INVERSE_LINK.get(link_type)
    if inverse and bpq.exists(TASK(), linked_task):
...
def get_report_tasks(scope="all", status_filter="open", priority=None, search=None,
    ...
    if proj_names:
        ...
        scope_projects = bpq.get_all(PROJECT(), pluck="name") if acc is None else list(acc)
    completed = set()
    ...
    total = len(bpq.get_all(TASK(), filters=db_filters, or_filters=or_filters, pluck="name"))
    ...
    for t in tasks:
        p = t["project"]
        if p not in pname:
            pname[p] = bpq.get_value(PROJECT(), p, "project_name") or p
...
def get_widget_data(config):
    ...
    if group_by == "status":
        ...
        color_sources = proj_names if proj_names else [p["name"] for p in bpq.get_all(PROJECT(), fields=["name"])]
        ...
        if pkeys:
            for p in bpq.get_all(PROJECT(), filters={"name": ["in", pkeys]}, fields=["name", "project_name", "project_color"]):
...
def get_team_dashboard(team):
    ...
    owned_projects = [] if _owned_filters is NO_ACCESSIBLE_PROJECTS else bpq.get_all(
        PROJECT(),
    ...
    if doctype == TASK() and fieldname == "status":
        ...
        if project:
            ...
            return [s.get("name") for s in _parse_json(
                bpq.get_value(PROJECT(), project, "workflow_states"), []
            ) if s.get("name")]

        accessible = get_accessible_projects()
        filters = {"name": ["in", list(accessible)]} if accessible is not None else {}
        rows = bpq.get_all(PROJECT(), filters=filters, pluck="workflow_states")
        seen, out = set(), []
        for raw in rows:
