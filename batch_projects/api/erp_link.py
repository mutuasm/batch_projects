...
def link_erpnext_project(project, erpnext_project):
    ...
    claimed_by = bpq.get_value(PROJECT(), {"erpnext_project": erpnext_project}, "name")
...
def search_erpnext_projects(txt=""):
    ...
    already_linked = set(bpq.get_all(
...
def _dedupe_project_name(base_name: str, so_name: str) -> str:
    ...
    if not bpq.exists(PROJECT(), {"project_name": name}):
        return name
    ...
    n = 2
    candidate = f"{name} ({n})"
    while bpq.exists(PROJECT(), {"project_name": candidate}):
        n += 1
        candidate = f"{name} ({n})"
    return candidate
...
def _derive_project_key(source_name: str) -> str:
    ...
    key = base
    n = 2
    while bpq.exists(PROJECT(), {"key": key}):
        key = f"{base}{n}"
        n += 1
    return key
...
def create_project_from_sales_order(sales_order, template=None, tasks_from_items=1, project_name=None):
    ...
    # Stamp both sides.
    bpq.set_value(PROJECT(), bp_project, "source_sales_order", so.name, update_modified=False)
...
def _create_bp_project_from_source(*, source_doctype, source_name, source_field,
    ...
    bpq.set_value(PROJECT(), bp_project, source_field, source_name, update_modified=False)
...
def _projects_referencing(doctype: str, name: str) -> set:
    ...
    if field:
        projects |= set(bpq.get_all(PROJECT(), filters={field: name}, pluck="name"))
    rows = frappe.db.sql(
...
def _auto_link_erpnext_project(project):
    ...
    bpq.set_value(PROJECT(), project, "erpnext_project", erp_doc.name)
    doc.add_comment(
        "Comment",
